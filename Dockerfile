...
RUN echo "http://dl-cdn.alpinelinux.org/alpine/v$(cat /etc/alpine-release | cut -d'.' -f1,2)/community" >> /etc/apk/repositories \
    && apk update \
    && apk add --no-cache libc6-compat openssl compat-openssl11
WORKDIR /app
...
# Create a directory for the sqlite database to ensure we can mount a volume there
...
ENV PORT 3000
# Update the DATABASE_URL to verify it points to a location we can mount
